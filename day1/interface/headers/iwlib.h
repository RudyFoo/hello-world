...
void iw_print_value_name(unsigned int		value,
		    const char *		names[],
		    const unsigned int		num_names);
void iw_print_ie_wpa(unsigned char *iebuf, int buflen);
void iw_print_gen_ie(unsigned char *buffer, int buflen);
void iw_print_ie_wpa_2(char *result, unsigned char *iebuf, int buflen);
void iw_print_gen_ie_2(char *result, unsigned char *buffer, int buflen);
void save_scanning_token(struct stream_descr *stream,	/* Stream of events */
		     struct iw_event *		event,	/* Extracted token */
		     struct iwscan_state *	state,
		     struct iw_range *	iw_range,	/* Range info */
		     int has_range,
             char *result, int result_len);
void save_scanning_ssid(struct stream_descr *stream,	/* Stream of events */
		     struct iw_event *		event,	/* Extracted token */
		     struct iwscan_state *	state,
		     struct iw_range *	iw_range,	/* Range info */
		     int has_range,
...
int get_scanning_info(int skfd, char *ifname, int TimeOut, char *result, int result_len);
int get_scanning_ssid(int skfd, char *ifname, int TimeOut, char *result, int result_len);
